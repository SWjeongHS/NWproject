...
int main(int argc, char *argv[]){
    ...
    const char localName[] = "C";
    send(sock,localName,strlen(localName),0);
    FILE * file13 = fopen("13daegu.csv","rt");
    ...
    FILE * file14 = fopen("14daegu.csv","rt");
    char line2 [1024];
    ...
    FILE * file22 = fopen("22daegu.csv","rt");
    char line3 [1024];
    ...
    while(1){
        requestData[0]='\0';
        recv(sock,requestData,sizeof(requestData),0);
        printf("%s",requestData);
        if(strcmp(requestData,"22")==0){
            ssize_t sentBytes22 = send(sock,file_buffer22,file_size22,0);
            if(sentBytes22==-1){
                error_handling("22read() error!");
            }
            else{
