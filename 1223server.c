...
{
    char *token;
    char tmpyear[4]; //파일의 첫번째를 저장하는 값
    ssize_t sendByte=0;
    ...
    char tmpsendyear[100], tmpptdname[20], tmpprice[5];
    int matcheddata[66296]={0,};
    int yearRight = 0;
    int count = 0;
    ...
    while(count < 66300){
        ...
        for(int i=0; i<6; i++)
        {
            ...
            if(token != NULL)
            {
                if(i==0)
                {
                    strcpy(tmpyear,token);
                    if(atoi(tmpyear)/10000==atoi(year)){
                        yearRight = 1;
                    }
                    strcat(tmpsendyear,token);
                    strcat(tmpsendyear,",");
                }
                if(i==2) //상품명
                {
                    strcpy(tmpptdname,token);
                    strcat(tmpsendyear,token);
                    strcat(tmpsendyear,",");
                }
                if(i==4) //가격
                {
                    strcpy(tmpprice,token);
                    strcat(tmpsendyear,token);
                    strcat(tmpsendyear,",");
                }
                ...
                if(i==5) //상품코드
                {
                    ...
                    if(strcmp(token,pdtnum)==0)
                    {
                        ...
                        {   
                            matcheddata[count]=1;
                            // strcat(sendyear,tmpsendyear);
                            // strcat(sendyear,"\n");
                            // ssize_t bytesend;
                            // sendByte += strlen(tmpsendyear);
                            // bytesend = send(clnt_sock,tmpsendyear,strlen(tmpsendyear),0);
                            // printf("%s\n",tmpsendyear);
                            // sendByte += bytesend;
                            // printf("%ld",sendByte);
                            // if(bytesend<0)
    ...
    while (fgets(line, 1024, file) != NULL) {
        ...
        if(matcheddata[count]==1){
            printf("%s\n",line);
            send(clnt_sock,line,filesize,0);
        }
        count++;
    }
...
// void sendCClient(void*arg)
// {
//     if(strcmp(requestlocal,"20")==0&&is22made[0]==0){
//         send(clnt_sock,requestlocal,sizeof(requestlocal),0);
//         receiveAndSaveFile(clnt_sock,"Receive22Busan.csv");
//         is22made[0]=1;
//     }
//     else if(strcmp(requestlocal,"21")==0&&is22made[1]==0){
//         send(clnt_sock,requestlocal,sizeof(requestlocal),0);
//         receiveAndSaveFile(clnt_sock,"Receive22Chungbuk.csv");
//         is22made[1]=1;
//     }
//     else if(strcmp(requestlocal,"22")==0&&is22made[2]==0){
//         send(clnt_sock,requestlocal,sizeof(requestlocal),0);
//         receiveAndSaveFile(clnt_sock,"Receive22Daegu.csv");
//         is22made[2]=1;
//     }
//     else if(strcmp(requestlocal,"23")==0&&is22made[3]==0){
//         send(clnt_sock,requestlocal,sizeof(requestlocal),0);
//         receiveAndSaveFile(clnt_sock,"Receive22Daejeon.csv");
//         is22made[3]=1;
//     }
//     else if(strcmp(requestlocal,"24")==0&&is22made[4]==0){
//         send(clnt_sock,requestlocal,sizeof(requestlocal),0);
//         receiveAndSaveFile(clnt_sock,"Receive22Gangwon.csv");
//         is22made[4]=1;
//     }
//     else if(strcmp(requestlocal,"25")==0&&is22made[5]==0){
//         send(clnt_sock,requestlocal,sizeof(requestlocal),0);
//         receiveAndSaveFile(clnt_sock,"Receive22Gwangju.csv");
//         is22made[5]=1;
//     }
//     else if(strcmp(requestlocal,"26")==0&&is22made[6]==0){
//         send(clnt_sock,requestlocal,sizeof(requestlocal),0);
//         receiveAndSaveFile(clnt_sock,"Receive22Gyeongi.csv");
//         is22made[6]=1;
//     }
//     else if(strcmp(requestlocal,"27")==0&&is22made[7]==0){
//         send(clnt_sock,requestlocal,sizeof(requestlocal),0);
//         receiveAndSaveFile(clnt_sock,"Receive22Jeju.csv");
//         is22made[7]=1;
//     }
//     else if(strcmp(requestlocal,"28")==0&&is22made[8]==0){
//         send(clnt_sock,requestlocal,sizeof(requestlocal),0);
//         receiveAndSaveFile(clnt_sock,"Receive22Jeonju.csv");
//         is22made[8]=1;
//     }
//     else if(strcmp(requestlocal,"29")==0&&is22made[9]==0){
//         send(clnt_sock,requestlocal,sizeof(requestlocal),0);
//         receiveAndSaveFile(clnt_sock,"Receive22Seoul.csv");
//         is22made[9]=1;
//     }
//     else if(strcmp(requestlocal,"30")==0&&is22made[10]==0){
//         send(clnt_sock,requestlocal,sizeof(requestlocal),0);
//         receiveAndSaveFile(clnt_sock,"Receive22Sejong.csv");
//         is22made[10]=1;
//     }
